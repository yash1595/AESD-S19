...
void FingerPrintInit(void)
{
    ...
    UARTConfigSetExpClk(UART7_BASE, SYSTEM_CLOCK, 9600,
    ...
    if(param==CMD_ID)
        {
            ...
            //check_id = (ResponseArray[8] & 0x00FF)<<8;
            check_id = ResponseArray[4];
            Value = ResponseArray[8];
            return Value;
        }
...
//    uint16_t check_id = CommandResponse(CMD_ID);
//    if(((check_id & 0XFF00)>>8)!=0x30)
   ...
   #if (TIVA_C == YES)
       UARTprintf("Valid ID\n");
       UARTprintf("%x",check_id);
     //  UARTprintf("ID:%x",(check_id & 0X00FF));
   #else
       LOG_INFO("Valid ID\n");
       LOG_INFO("ID:%x",(check_id & 0X00FF));
   #endif
...
void fingerprint_task(void *pvParameters)
{
    ...
    while(1)
    {
      ...
      if(xQueueSend(log_q,(void *)&finger_info, 100) != pdTRUE)
       {
           UARTprintf("\nLog Error");
       }
    }
